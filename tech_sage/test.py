...
COMMANDS = {'add_name': ['add_name', 'Додавання нового контакту у довідник'],
            'add_user': ['add_user', 'Додавання нового користувача'],
            'authenticate_user': ['authenticate_user', 'Вхід в аккаунт'],
            'add_phone': ['add_phone Name',
                          'Додавання телефонного номеру до контакту Name.\nКожен контакт може мати кілька номерів'],
            'add_birthday': ['add_birthday Name',
                             'Додавання для контакта Name дня народження у форматі РРРР-ММ-ДД.\nКожен контакт має тільки один день народження.\nТакож застосовується для зміни дня народження'],
            'add_email': ['add_email Name',
                          'Додавання адреси електроної пошти для контакта Name.\nКожен контакт має тільки один e-mail.\nТакож застосовується для зміни e-mail'],
            'add_address': ['add_address Name',
                            'Додавання адреси для контакта Name.\nКожен контакт має тільки одну адресу.\nТакож застосовується для зміни адреси'],
            'find_record_by_trem': ['find_record_by_trem text',
                                    "Пошук рядку 'text' у всіх полях телефонного довідника"],
            ...
            'when': ['when Number',
                     'Виводить на екран список контактів, у яких день народження впродовж "Number" днів від сьогодні'],
            'sort_files': ['sort_files Path',
                           'Сортує файли у папці "Path" на вашому диску по папках в залежності від типу файлу'],

            'help': ['help', 'Виклик довідника команд, що вміє цей бот'],
            'load': ['load',
                     'Завантаження довідника з файла на диску. \nПерезапише зміни, що були внесені та не збережені у файл.\nТакож відбувається автоматично при запуску програми'],
            'save': ['save',
                     'Зберігання змін у довіднику у файл на диску.\nТакож відбувається автоматично при закінченні роботи з програмою'],
...
class NoteRecord(Record):
    ...
    def find_notes_by_term(self, term):
        return [note for note in self.notes if
                term.lower() in note.value.lower() or any(term.lower() in tag.lower() for tag in note.tags)]
...
class Controller():
    ...
    def do_authenticate_user(self, username, password):
        if self.book.authenticate_user(username, password):
            self.view.render(f"Користувача {username} успішно ідентифіковано")
        else:
            self.view.render("Помилка індитифікації")
    ...
    def do_days_to_birthday(self, line,
                            when=9999):  # >>>birthday John (до дня народження контакту John, залишилось 354 днів)
        if when == 9999:
            table = Table(show_header=True, header_style="bold magenta", border_style='bold violet')
        else:
...
class CommandValidator(Validator):
    def validate(self, document):
        ...
        if text.startswith("add_user"):
            x = text.strip().split(" ")
            if len(x) < 4:
                raise ValidationError(message="Введіть: add_user <Name> <Username> <Password>",
...
def handle_command(controller, command):
    ...
    elif command.lower().startswith("add_user"):
        return controller.do_add_user()
    elif command.lower().startswith("authenticate_user"):
        return controller.do_authenticate_user()
